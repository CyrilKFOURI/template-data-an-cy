...
def kpi_count_share_quarterly_market(df: pd.DataFrame, var_col: str) -> pd.DataFrame:
    out = df.copy()
    if out.empty:
        return pd.DataFrame()
...
def update_view5_kpi11(_refresh_clicks: int, status_value: str, source_value: str, variable: str):
    ...
    if source_value == "market":
        ...
        kpi11_table_long = kpi_count_share_quarterly_market(market_ready, market_var)
        fig = plot_kpi_share(kpi11_table_long, market_var)
        # We still return a table, but maybe keep the wide one for display? 
        # Actually let's use the wide one for the table display if requested, 
        # but the user said "mm chose que portfolio". 
        # Portfolio shows a long table in the build_table below.
