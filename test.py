...
def lookup_kpi11(source, status=None, reg_type=None, owner=None, variable=None, top_n=None, period_mode=None):
    ...
    mask = (
        ...
        (CACHE_KPI11["VARIABLE_FILTER"]    == var) &
    ...
    return res.drop(columns=[col for col in _KPI11_FILTER_COLS if col in res.columns], errors="ignore")
...
def fast_update_view5_kpi11(_n, status, market_reg, source, variable, top_n, period_mode, market_owner):
    ...
    _known_data_cols = {"COUNTRY", "YEAR", "PERIOD", "VOLUME", "TOTAL", "SHARE", "VOLUME_YTD", "SHARE_YTD"}
    var_col = next((c for c in df11.columns if c not in _known_data_cols), variable)

    if "VOLUME_YTD" in df11.columns:
        plot_df = df11.rename(columns={"VOLUME_YTD": "VOLUME", "SHARE_YTD": "SHARE"}).copy()
    else:
        plot_df = df11.copy()

    try:
        fig = fmd.plot_kpi_share(plot_df, var_col, period_mode or "monthly")
    except Exception:
        fig = empty_fig

    table = fmd.build_table(df11, page_size=15, heatmap=True, table_id="v5-kpi11-table")
...
def fast_update_view5_country_breakdown(_n, status, source, variable, metric_mode, top_n, period_mode):
    ...
    table = fmd.build_table(df_cb, page_size=15, heatmap=True)
    title = f"Top {variable} per country – {source} – {metric_mode}"
    return html.Div([html.H4(title, className="panel-title"), table])
