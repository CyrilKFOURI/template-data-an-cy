...
def prepare_data_set(df: pd.DataFrame) -> pd.DataFrame:
    # Minimal transformations: just datetime conversion as requested
    ...
    # We still need YEAR and MONTH columns for the dashboard's internal filtering logic
    # but we extract them directly without any modification.
...
def kpi_selected_volume(
    ...
    # No more status override! Respect the status provided (e.g. 'IN FLEET')
...
def filter_base(df: pd.DataFrame, country: str, year: int | str, month: str | int | None = "ALL", status: str = "IN FLEET", bike_or_car: str = "CAR", date_mode: str = "NONE") -> pd.DataFrame:
    ...
    # Apply filters in order, exactly as in the notebook query
    if status and status != "ALL":
        subset = subset[subset["NOVA_ASSET_STATUS"] == status]
    ...
    # Date Mode logic: exactly mimic df['DELIVERY_DATE'].dt.month == df['COB_DATE'].dt.month
...
def get_view1_metrics_cached(
    ...
    return {
        ...
        "month1": month1,
        "month2": month2,
        "kpi1_volume": kpi_selected_volume(df, kpi1_country, kpi1_year, month1, status="IN FLEET", bike_or_car=kpi_common_bike_or_car, date_mode=dm1),
        "kpi2_volume": kpi_selected_volume(df, kpi2_country, kpi2_year, month2, status="IN FLEET", bike_or_car=kpi_common_bike_or_car, date_mode=dm2),
        "kpi3_volume": kpi_selected_volume(df, kpi3_country, kpi3_year, month3, status="IN FLEET", bike_or_car=kpi_common_bike_or_car, date_mode=dm3),
        "kpi4_volume": kpi_selected_volume(df, kpi4_country, kpi4_year, month4, status="IN FLEET", bike_or_car=kpi_common_bike_or_car, date_mode=dm4),
        "kpi5_volume": kpi_selected_volume(df, kpi5_country, kpi5_year, month5, status="IN FLEET", bike_or_car=kpi_common_bike_or_car, date_mode=dm5),
        "kpi6_volume": kpi_selected_volume(df, kpi6_country, kpi6_year, month6, status="IN FLEET", bike_or_car=kpi_common_bike_or_car, date_mode=dm6),
    }
...
def build_table(df_table: pd.DataFrame, page_size: int = 15) -> dash_table.DataTable:
    ...
    # Add bold styling for TOTAL row
    ...
    if total_row_index is not None:
        row_style.append({"if": {"row_index": total_row_index}, "fontWeight": "bold", "backgroundColor": "#e6eaf0"})
...
def build_html_table_from_df(df_table: pd.DataFrame, title: str) -> str:
        ...
        # Handle volume tables: add total row and remove % from total row only
        is_volume_table = "volume" in title.lower()
        if is_volume_table:
            # Add total row for numeric columns
            numeric_cols = table_df.columns[1:]
...
def build_kpi7_table_with_total(
    ...
    if metric_mode == "share":
        ...
        if not volume_pivot.empty:
            table_df["TOTAL"] = volume_pivot.sum(axis=1).reindex(table_df.index, fill_value=0)
            volume_totals = volume_pivot.sum(axis=0).reindex(metric_cols, fill_value=0)
            total_row = pd.DataFrame([volume_totals], index=["TOTAL"])
            total_row["TOTAL"] = float(volume_pivot.to_numpy().sum())
        ...
        total_row = pd.DataFrame([table_df[metric_cols + ["TOTAL"]].sum(axis=0)], index=["TOTAL"])
        table_df = pd.concat([table_df, total_row])
...
def build_view2_table_with_total(table_df: pd.DataFrame, metric_mode: str) -> pd.DataFrame:
    display_df = table_df.copy()
    metric_cols = [column for column in display_df.columns if column not in ["YEAR", "MONTH"]]

    if not display_df.empty and should_show_total_column(display_df, metric_cols, share_mode=(metric_mode == "share")):
        display_df["TOTAL"] = display_df[metric_cols].sum(axis=1)
        total_row = {"YEAR": "TOTAL", "MONTH": "ALL"}
        for column in metric_cols + ["TOTAL"]:
            total_row[column] = display_df[column].sum()
        display_df = pd.concat([display_df, pd.DataFrame([total_row])], ignore_index=True)

    if metric_mode == "share":
        for column in metric_cols:
            display_df[column] = display_df[column].map(lambda value: percent_or_na_precision(cast(float | None, value), 1) if pd.notna(value) else "N/A")
...
def build_view1_download_report(
    ...
    kpi1_val = kpi_lease_under_25(df, kpi1_country, kpi1_year, kpi1_month, kpi_common_bike_or_car, kpi1_dm)
    kpi2_val = kpi_lease_25_30(df, kpi2_country, kpi2_year, kpi2_month, kpi_common_bike_or_car, kpi2_dm)
    diesel_non = kpi_diesel_non_diesel(df, kpi3_country, kpi3_year, month3, kpi_common_bike_or_car, kpi3_dm)
    hybrid_val = kpi_hybrid_share(df, kpi4_country, kpi4_year, month4, kpi_common_bike_or_car, kpi4_dm)
    ev_val = kpi_ev_share(df, kpi5_country, kpi5_year, month5, kpi_common_bike_or_car, kpi5_dm)
    pv_lcv_val = kpi_pv_lcv(df, kpi6_country, kpi6_year, month6, kpi_common_bike_or_car, kpi6_dm)
...
def build_view2_download_report(
    ...
    title_mode = "share" if metric_mode == "share" else "volume"
    fig.update_layout(
        title=f"Production by fuel type ({title_mode}) by {x_title.lower()}<br><sup>{period_label} | {country} | {bike_or_car}</sup>",
        xaxis_title=x_title,
        yaxis_title=y_title,
        template="plotly_white",
        height=520,
...
def kpi8_production_ytd(
    ...
    if metric_mode == "share":
        ...
        pivot = grouped.pivot(index=["YEAR", "MONTH"], columns="POWER_CATEGORY_3", values="SHARE").fillna(0)
        y_title = "Share (%)"
    else:
        pivot = grouped.pivot(index=["YEAR", "MONTH"], columns="POWER_CATEGORY_3", values="VOLUME").fillna(0)
        y_title = "Volume"

    pivot = pivot.reset_index().sort_values(["YEAR", "MONTH"]).reset_index(drop=True)
    return pivot, y_title, "Month", f"{year}"
...
def plot_kpi_share(df_plot: pd.DataFrame, col: str, period_mode: str = "quarterly") -> go.Figure:
    ...
    # 3. Identify Latest Period for Treemap
    times = sorted(agg["TIME"].unique())
    latest_time = times[-1]
    latest_df = agg[agg["TIME"] == latest_time].sort_values("VOLUME", ascending=False)
    
    # 4. Identify Top 5 Categories for Trend
    overall_rank = agg.groupby(col)["VOLUME"].sum().sort_values(ascending=False)
    top_cats = overall_rank.index[:5].tolist()
    
    # 5. Calculate CR3 (Concentration Ratio of Top 3) for Trend
    cr3_data = []
    for t in times:
        t_df = agg[agg["TIME"] == t].sort_values("VOLUME", ascending=False)
        cr3 = t_df.head(3)["SHARE"].sum()
        cr3_data.append({"TIME": t, "CR3": cr3})
    cr3_df = pd.DataFrame(cr3_data)

    # 6. Create Dual Visualization
    fig = make_subplots(
        rows=1, cols=2,
        column_widths=[0.4, 0.6],
        specs=[[{"type": "treemap"}, {"secondary_y": True}]],
        subplot_titles=(f"Current State ({latest_time})", "Evolution of Top Segments & Concentration")
    )
    ...
    # Right: CR3 Line (Concentration Index)
    fig.add_trace(
        go.Scatter(
            x=cr3_df["TIME"], y=cr3_df["CR3"],
            name="CR3 Ratio (Top 3 Sum)",
            mode="lines+markers",
            line=dict(color="black", width=4, dash="dot"),
            hovertemplate="Top 3 Share: %{y:.1f}%"
        ),
        row=1, col=2, secondary_y=False
    )

    # Right: Total Volume on secondary axis
    fig.add_trace(
        go.Scatter(
            x=total_per_time["TIME"], y=total_per_time["TOTAL_VOLUME"],
            name="Total Volume",
            mode="lines",
    ...
    fig.update_yaxes(title_text="Share (%)", range=[0, 100], secondary_y=False, row=1, col=2)
    fig.update_yaxes(title_text="Total Volume", secondary_y=True, row=1, col=2, showgrid=False)
...
def kpi_top_brand_vs_market(df_portfolio: pd.DataFrame, df_market: pd.DataFrame, var_col_portfolio: str, var_col_market: str, asset_status: str, bev_only: bool = False) -> pd.DataFrame:
    ...
    df_port["QUARTER"] = pd.to_datetime(df_port["COB_DATE"]).dt.to_period('Q').astype(str)
    ...
    # Available quarters in market
    available_quarters = set(df_mkt["QUARTER"].unique())
    df_port = df_port[df_port["QUARTER"].isin(available_quarters)]

    # Portfolio aggregation (size() to count rows)
    port = (
        df_port.groupby(["COUNTRY", "QUARTER", var_col_portfolio])
        .size()
    ...
    # Get top brand per country/quarter in portfolio
    port_top = (
    ...
    # Market aggregation
    mkt_agg = (
        df_mkt.groupby(["Country/Territory-Number", "QUARTER", var_col_market])["volume"]
        .sum()
        .reset_index(name="volume_market")
    )
    mkt_agg["total_market"] = mkt_agg.groupby(["Country/Territory-Number", "QUARTER"])["volume_market"].transform("sum")
    mkt_agg["share_market"] = mkt_agg["volume_market"] / mkt_agg["total_market"]
    ...
    df_final = port_top.merge(
        mkt_agg, on=["COUNTRY", "QUARTER", "BRAND"], how="left"
    )
    df_final["ratio"] = df_final["share_portfolio"] / df_final["share_market"].replace(0, np.nan)
    return df_final
            ...
            html.Div(
                [
                    html.Div(
                        [
                            html.Div("Status", className="filter-label"),
                            dcc.Dropdown(id="v6-kpi13-status-filter", options=MARKET_STATUS_OPTIONS, value=None, placeholder="Select status", clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Variable", className="filter-label"),
                            dcc.Dropdown(id="v6-kpi13-variable-filter", options=MARKET_VARIABLE_OPTIONS, value=None, placeholder="Select variable", clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
...
def download_view2_html(_n_clicks: int, country: str, year: int | str, metric_mode: str, bike_or_car: str, v2_date_mode_filter: str):
...
def update_view2_kpi8(_refresh_clicks: int, country: str, year: int | str, metric_mode: str, bike_or_car: str, v2_date_mode_filter: str):
    asset_status = "IN FLEET"
    if has_missing_filters(country, year, asset_status, metric_mode, bike_or_car, v2_date_mode_filter):
        empty_fig = go.Figure()
        empty_fig.update_layout(title="Select the filters then click Refresh")
...
def update_v5_variable_options(source):
    if source == "market":
...
def update_view6_kpi13(_refresh_clicks: int, status_value: str, variable: str):
...
def build_interactive_complete_report(
    ...
    for year in selected_years:
        for gran in selected_periods:
            ...
            for sub in subs:
                # Map sub to month integer for calculation
                month_val = sub
                if gran == "quarterly":
