...
_raw_v1   = _load_cache(os.path.join(BASE_DATA, "merged_v1_pv_lcv_fixed.parquet"), os.path.join(BASE_DATA, "view1", "kpis"), ".parquet")
...
def lookup_kpi10(country, start_year, end_year, status, bike_or_car, period_mode="quarterly"):
    """Return KPI10 pivot [Period × top-10 models + Total] for the given EOC year range and period mode."""
    ...
    s  = str(status)      if status      else "IN FLEET"
    b  = str(bike_or_car) if bike_or_car else "ALL"
    pm = str(period_mode) if period_mode else "quarterly"
    try:
        sy = int(start_year)
        ey = int(end_year)
    except (TypeError, ValueError):
    ...
    # Support both new schema (EOC_YEAR + PERIOD_MODE_FILTER) and legacy (YEAR_FILTER)
    year_col = "EOC_YEAR" if "EOC_YEAR" in CACHE_KPI10.columns else "YEAR_FILTER"
    has_pm   = "PERIOD_MODE_FILTER" in CACHE_KPI10.columns

    mask = (
        (CACHE_KPI10["COUNTRY_FILTER"]      == c) &
        (CACHE_KPI10["ASSET_STATUS_FILTER"] == s) &
        (CACHE_KPI10["BIKE_OR_CAR_FILTER"]  == b) &
        (CACHE_KPI10[year_col]              >= sy) &
        (CACHE_KPI10[year_col]              <= ey)
    )
    if has_pm:
        mask = mask & (CACHE_KPI10["PERIOD_MODE_FILTER"] == pm)

    res = CACHE_KPI10[mask]
    if res.empty:
        return pd.DataFrame()

    # Pivot: PERIOD → rows, MARKET_MODEL → columns
    period_col = "PERIOD" if "PERIOD" in res.columns else year_col
    pivot = res.pivot_table(
        index=period_col, columns="MARKET_MODEL", values="COUNT",
        aggfunc="sum", fill_value=0,
    )

    # Keep top 10 models by total across the selected range
    top10 = pivot.sum().nlargest(10).index
    pivot = pivot[top10]

    # Add Total row
    total_row = pd.DataFrame([pivot.sum()], index=pd.Index(["Total"], name=period_col))
    pivot = pd.concat([pivot, total_row])

    table = pivot.reset_index().rename(columns={period_col: "Period"})
...
def fast_update_view4_kpi10(_n, country, start_year, end_year, status, bike_or_car):
    empty_fig = go.Figure()

    if fmd.has_missing_filters(country, start_year, end_year, status, bike_or_car):
    ...
    table = lookup_kpi10(
        country or "ALL", start_year, end_year,
        status or "IN FLEET", bike_or_car or "ALL",
    )
    ...
    years_rows = table.loc[~is_total, period_col].tolist()

    if not years_rows or not model_cols:
    ...
    for model in model_cols:
        fig.add_trace(
            go.Bar(x=years_rows, y=table.loc[~is_total, model].astype(float).tolist(), name=str(model)),
            secondary_y=False,
        )
    year_totals = table.loc[~is_total, model_cols].sum(axis=1)
    fig.add_trace(
        go.Scatter(x=years_rows, y=year_totals.tolist(), mode="lines+markers", name="TOTAL"),
        secondary_y=True,
    )
    fig.update_layout(
        title=f"KPI 10 – Top models at EOC ({country}, {start_year}–{end_year}, {status}, {bike_or_car})",
        barmode="group", template="plotly_white", height=460, xaxis_title="Year",
    )
