...
def enrich_with_models(df: pd.DataFrame) -> pd.DataFrame:
    global MODELS_DF
    _enrich_cols = ["MARKET_MODEL", "MARKET_BODY_GROUP", "CDN_CLF_SEGMENT", "CDN_CLF_BODY_TYPE"]
    ...
    if MODELS_DF.empty:
        # models.parquet not loaded — try to build it from nova if the cols exist here
        present = [c for c in _enrich_cols if c in out.columns and out[c].notna().any()]
        if present:
            build_cols = ["BRAND_UPDATE", "MODEL"] + present
            mdf = (
                out[build_cols]
                .dropna(subset=["BRAND_UPDATE", "MODEL"])
                .copy()
            )
            mdf["MODEL"] = mdf["MODEL"].astype(str).str.strip()
            mdf["BRAND_UPDATE"] = mdf["BRAND_UPDATE"].astype(str).str.strip()
            mdf = mdf.drop_duplicates(subset=["BRAND_UPDATE", "MODEL"])
            try:
                mdf.to_parquet(str(DEFAULT_MODELS_PATH), index=False)
                MODELS_DF = mdf
                print(f"[enrich_with_models] Created models.parquet: {len(mdf):,} unique (brand, model)")
            except Exception as e:
                print(f"[enrich_with_models] Warning: could not save models.parquet: {e}")
      
        return out

    # models.parquet is available — skip if df is already fully enriched
    if all(c in out.columns and out[c].notna().any() for c in ["MARKET_MODEL", "MARKET_BODY_GROUP"]):
        return out

    out = out.drop(columns=[c for c in _enrich_cols if c in out.columns], errors="ignore")
...
def prepare_data_set(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "CLS_VEHICLE_TYPE" in df.columns:
        _cls_num_map = {
            "1": "PV",
            "2": "Station wagon",
            "3": "Vans",
            ...
            "8": "CV with truck registration",
        }
        def _normalize_cls_vehicle_type(val: str) -> str:
            if val in _cls_num_map:
                return _cls_num_map[val]
            if val in ("PV", "LCV", "Station wagon", "Vans", "Medium-duty CV",
                       "HCV", "Minibus / People Mover", "CV with truck registration"):
                return val
            try:
                key = str(int(float(val)))
                return _cls_num_map.get(key, "Not Identified")
            except (ValueError, TypeError):
                return val

        df["CLS_VEHICLE_TYPE"] = df["CLS_VEHICLE_TYPE"].apply(_normalize_cls_vehicle_type)
...
def kpi9_1_type_share_period(
    ...
    total = subset.groupby("PERIOD")["VEHICLE_ID"].nunique()
    if vehicle_type_value == "ALL":
        selected = total.copy()
    else:
        selected = subset[subset[body_group_col] == vehicle_type_value].groupby("PERIOD")["VEHICLE_ID"].nunique()
...
def kpi9_2_power_category_per_type_period(
    ...
    grouped = subset.groupby(["PERIOD", "POWER_CATEGORY"])['VEHICLE_ID'].nunique().reset_index(name="COUNT")
...
def kpi_top_brand_vs_market(df_portfolio: pd.DataFrame, df_market: pd.DataFrame, var_col_portfolio: str, var_col_market: str, asset_status: str, reg_type_portfolio: list | str = "ALL", reg_type_market: list | str = "ALL", bev_only: bool = False, top_n: str = "ALL", period_mode: str = "quarterly") -> pd.DataFrame:
    ...
    port["COUNTRY"] = port["COUNTRY"].astype(str).str.strip().str.upper().map(country_map).fillna(
        port["COUNTRY"].astype(str).str.strip()
    )
...
OWNER_TYPE_OPTIONS: list[dict] = [{"label": "All", "value": "ALL"}]
...
if not market_df.empty and "Ownertype" in market_df.columns:
    ...
    OWNER_TYPE_OPTIONS = [{"label": "All", "value": "ALL"}] + [
        {"label": v, "value": v} for v in _owner_vals if v not in ("ALL", "nan", "")
    ]
