...
def build_view2_table_with_total(table_df: pd.DataFrame, metric_mode: str) -> pd.DataFrame:
    ...
    res_df = table_df.copy()
    metric_cols = [c for c in res_df.columns if c not in ["YEAR", "MONTH"]]
    ...
    for col in metric_cols:
        try:
            ...
            total_row[col] = "-"
    return pd.concat([res_df, pd.DataFrame([total_row])], ignore_index=True)
...
def build_view1_download_report(
    ...
    month4 = resolve_month_value(kpi4_country, kpi4_year, kpi4_month)
    month5 = resolve_month_value(kpi5_country, kpi5_year, kpi5_month)
    month6 = resolve_month_value(kpi6_country, kpi6_year, month_value if kpi6_month == "ALL" else kpi6_month)
...
@app.callback(
    ...
    [State("month-filter", "value"), State("period-mode-filter", "value")] + [State(f"kpi{i}-month-filter", "value") for i in range(1, 7)]
)
def sync_granularity_filters(gran, current_main_val, current_p_mode, *current_card_vals):
    ...
    if gran == "yearly":
        ...
        opts = quarter_opts
        main_val = current_main_val if any(o["value"] == current_main_val for o in quarter_opts) else "ALL"
        card_vals = [v if any(o["value"] == v for o in quarter_opts) else "ALL" for v in current_card_vals]
        label = "Quarter"
    else:
        opts = month_opts
        main_val = current_main_val if any(o["value"] == current_main_val for o in month_opts) else "ALL"
        card_vals = [v if any(o["value"] == v for o in month_opts) else "ALL" for v in current_card_vals]
        label = "Month"

    return [opts, main_val, label, p_mode] + [opts] * 6 + card_vals
