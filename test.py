...
def _code_to_name_map() -> dict[str, str]:
    ...
    for name in ALL_COUNTRY_NAMES:
        files = sorted(glob.glob(os.path.join(str(DATA_FOLDER), f"*{name}*.parquet")))
        if not files:
            continue
        try:
...
def _codes_in_parquets() -> set[str]:
    ...
    codes: set[str] = set()
    for _, dirnames, _ in os.walk(str(BASE_OUT)):
        for d in dirnames:
            if d.startswith("country="):
                raw_code = d[len("country="):].replace("_", " ")
                codes.add(raw_code)
    return codes
...
def run() -> None:
    ...
    for code in sorted(codes_present):
        ...
        print("  KPI 1-6:")
        for p in sorted(glob.glob(os.path.join(str(BASE_OUT), "view1", "kpis", "*", f"country={safe}", "kpis.parquet"))):
            _fix_kpis(Path(p), df_raw, actual_bocs)

        print("  KPI 7:")
        for p in sorted(glob.glob(os.path.join(str(BASE_OUT), "view1", "kpi7", "*", f"country={safe}", "kpi7.parquet"))):
            _fix_kpi7(Path(p), df_raw, actual_bocs)

        print("  KPI 8:")
        for p in sorted(glob.glob(os.path.join(str(BASE_OUT), "view2", "kpi8", "*", f"country={safe}", "kpi8.parquet"))):
            _fix_kpi8(Path(p), df_raw, actual_bocs)
