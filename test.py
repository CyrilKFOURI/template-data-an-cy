...
# This dashboard never scans the raw NOVA parquets — it reads the small,
# already-filtered-and-enriched extract written by generate_use_case_1_precomputed.py.
PRECOMPUTED_PATH = BASE_DIR / "precomputed_use_case_1" / "use_case_1_data.parquet"
...
# =============================================================================
# Startup: read the precomputed extract (no raw NOVA scan, no re-enrichment) —
# built by generate_use_case_1_precomputed.py, which owns the loading and
# models.parquet enrichment logic this dashboard used to do at import time.
# =============================================================================

if PRECOMPUTED_PATH.exists():
    UC1_DF = pd.read_parquet(PRECOMPUTED_PATH)
else:
    UC1_DF = pd.DataFrame(columns=COLUMNS_TO_READ)
    print(f"WARNING: {PRECOMPUTED_PATH} not found. Run generate_use_case_1_precomputed.py first — "
          f"the dashboard will start with no data until then.")
...
def _rwiz_scoped_df(current_records, brand=None, model=None, bodytype=None, power=None, co2=None) -> pd.DataFrame:
    """Narrows the CURRENT portfolio state (already dashboard-filtered, COB-
    filtered, and replayed with every queued action so far) by whichever
    Remove Vehicles wizard fields are already picked. Shared by the live
    counter and by each step's option list, so a dropdown never offers a
    choice that would leave zero matching vehicles — and, because it reads
    the current portfolio rather than the untouched original, it can see
    (and remove) vehicles a prior Add Vehicles action already created."""
    d = _records_to_df(current_records)
    return _narrow_by_removal_filters(d, brand, model, bodytype, power, co2)


def count_matching_for_removal(current_records, brand, model=None, bodytype=None, power=None, co2=None) -> int:
    """Number of vehicles in the current portfolio state matching the Remove
    Vehicles wizard's brand/model/body type/power/CO2 picks. Drives the live
    counter shown throughout the wizard."""
    d = _rwiz_scoped_df(current_records, brand, model, bodytype, power, co2)
    return int(len(d))
...
# =============================================================================
# Persistent, order-sensitive portfolio state for simulation
#
# Instead of two independent "pending" lists (additions, removals) that both
# get combined against the untouched original every time Run Simulation is
# clicked, every Add/Remove action is appended to one ordered log, and the
# "current portfolio" is always the untouched original replayed through that
# log in order. This makes a later removal able to remove vehicles an earlier
# addition just created (and a later addition unaffected by an earlier
# removal), instead of the two being computed independently. Practical only
# because this dashboard's dataset is small (one country, one month).
# =============================================================================
...
def _df_to_records(df: pd.DataFrame) -> list[dict]:
    return [{k: _json_safe_value(v) for k, v in row.items()} for row in df.to_dict("records")]


def _records_to_df(records: list[dict] | None) -> pd.DataFrame:
    if not records:
        return pd.DataFrame()
...
def replay_portfolio(d_orig: pd.DataFrame, action_log: list[dict], cob_date: pd.Timestamp) -> pd.DataFrame:
    """Apply every queued add/remove action, in order, on top of the untouched
    original portfolio. Order-sensitive and cumulative: a removal queued after
    an addition can remove vehicles that addition just created."""
    current = d_orig.copy()
    if "ID_CONTRACT" in current.columns and not current.empty:
        next_id = int(pd.to_numeric(current["ID_CONTRACT"], errors="coerce").max() or 0) + 1
    else:
        next_id = 900000
    ...
    for entry in action_log or []:
        if entry.get("type") == "add":
            ...
            part = build_synthetic_vehicles(entry, cob_date, next_id)
            next_id += qty
            current = pd.concat([current, part], ignore_index=True)
        elif entry.get("type") == "remove":
            current = apply_removal_batches(current, [entry])
    return current
...
app.title = "Use Case 1 — Portfolio Heatmap (Precomputed)"
...
app.layout = html.Div(
    [
        ...
        dcc.Store(id="sim-action-log-store", data=[]),
        dcc.Store(id="current-portfolio-store", data=[]),
        ...
        html.Div(
            [
                html.Div(
                    [
                        html.H1("Use Case 1 — Portfolio Heatmap (Precomputed)",
                    ...
                    [
                        ...
                        html.Div(
                            ...
                            children=[
                                ...
                                html.Div(
                                    [
                                        _panel_title("Portfolio Changes — Applied in Order"),
                                        html.P(
                                            "Every addition and removal is applied immediately to the current "
                                            "portfolio, in the order shown below — a later removal can remove "
                                            "vehicles an earlier addition just created, and vice versa. Removing "
                                            "an entry here undoes just that step.",
                                            style={"fontSize": "12px", "color": "#718096", "margin": "0 0 14px"},
                                        ),
                                        html.Div(id="sim-action-log-list", children=[
                                            html.P("No changes yet. Use “+ Add Vehicles” or “− Remove Vehicles” to start.",
...
def _wiz_step_visibility(step):
    ...
    primary_label = "Add to Portfolio" if step == 4 else "Next"
...
@app.callback(
    ...
    Output("sim-action-log-store", "data"),
    ...
    State("f-country", "value"),
    State("sim-action-log-store", "data"),
    prevent_initial_call=True,
)
def _wiz_controller(open_n, back_n, primary_n, cancel_n, step,
                     ...
                     price, ltr, mtr, f_country, action_log):
    ...
    if "wiz-btn-primary" in trig:
        ...
        if step == 4:
            ...
            return ({**MODAL_OVERLAY_STYLE, "display": "none"}, 0, new_log, "", *RESET_VALUES)

    return (no_update,) * 13


# =============================================================================
# Callbacks — Portfolio changes log (combined, ordered add/remove actions)
# =============================================================================

@app.callback(
    Output("sim-action-log-list", "children"),
    Input("sim-action-log-store", "data"),
)
def _render_sim_action_log(action_log):
    action_log = action_log or []
    if not action_log:
        return html.P("No changes yet. Use “+ Add Vehicles” or “− Remove Vehicles” to start.",
                       style={"fontSize": "12px", "color": "#a0aec0", "fontStyle": "italic"})
    rows = []
    for i, entry in enumerate(action_log):
        is_add = entry.get("type") == "add"
        badge = html.Span("ADDED" if is_add else "REMOVED", style={
            "fontSize": "10px", "fontWeight": "700", "padding": "2px 8px", "borderRadius": "999px",
            "color": "#fff", "background": "#276749" if is_add else "#9b2c2c",
            "marginRight": "10px", "whiteSpace": "nowrap",
        })
        rows.append(html.Div(
            [
                html.Div([badge, html.Span(f"{i + 1}. {entry.get('summary', '')}",
                                            style={"fontSize": "13px", "color": "#1a1a2e"})],
                         style={"display": "flex", "alignItems": "center", "flex": "1"}),
                html.Button("Undo", id={"type": "sim-log-remove-btn", "index": i}, n_clicks=0, style=DANGER_BTN_STYLE),
            ],
...
@app.callback(
    Output("sim-action-log-store", "data", allow_duplicate=True),
    Input({"type": "sim-log-remove-btn", "index": ALL}, "n_clicks"),
    State("sim-action-log-store", "data"),
    prevent_initial_call=True,
)
def _remove_sim_action_log_item(n_clicks_list, action_log):
    ...
    try:
        ...
        return no_update
    action_log = action_log or []
    return [e for i, e in enumerate(action_log) if i != idx]


# =============================================================================
# Callbacks — Remove Vehicles wizard: cascading options
# Each dropdown is scoped to the CURRENT portfolio state (the untouched original
# replayed with every queued action so far — see current-portfolio-store) plus
# every field already picked earlier in the wizard, so it only ever offers
# choices that leave at least one matching vehicle, and it can offer (and
# remove) vehicles a prior Add Vehicles action already created.
# =============================================================================

@app.callback(
    Output("rwiz-brand", "options"),
    Input("btn-open-remove-wizard", "n_clicks"),
    State("current-portfolio-store", "data"),
    prevent_initial_call=True,
)
def _rwiz_brand_options(_open, current_records):
    d = _rwiz_scoped_df(current_records)
...
def _rwiz_model_options(brand, current_records):
    if not brand:
        return [], None
    d = _rwiz_scoped_df(current_records, brand)
...
def _rwiz_bodytype_options(brand, model, current_records):
    if not brand:
        return [], None
    d = _rwiz_scoped_df(current_records, brand, model)
...
def _rwiz_power_options(brand, model, bodytype, current_records):
    if not brand:
        return [], None
    d = _rwiz_scoped_df(current_records, brand, model, bodytype)
...
def _rwiz_co2_options(brand, model, bodytype, power, current_records):
    if not brand:
        return [], None
    d = _rwiz_scoped_df(current_records, brand, model, bodytype, power)
...
def _rwiz_count(brand, model, bodytype, power, co2, _open, current_records):
    if not brand:
        return "Select a brand to see how many vehicles match your selection."
    n = count_matching_for_removal(current_records, brand, model, bodytype, power, co2)
...
def _rwiz_qty_bounds(brand, model, bodytype, power, co2, current_records):
    ...
    that combination in the current portfolio state, so the user can never type
    more than what exists."""
    if not brand:
        return None, None
    available = count_matching_for_removal(current_records, brand, model, bodytype, power, co2)
...
@app.callback(
    Output("current-portfolio-store", "data"),
    Input("sim-action-log-store", "data"),
    Input("btn-refresh", "n_clicks"),
    State("f-country",    "value"),
    State("f-asset-status", "value"),
...
def _recompute_current_portfolio(action_log, _refresh, country, asset_status, brands, models, bodytypes, cob_store):
    """The current portfolio is always the untouched original replayed through
    every queued add/remove action, in order — recomputed whenever the action
    log changes (an action was queued or undone) or the dashboard filters are
    refreshed. This is the single "always know the current state" variable the
    simulation reads from, instead of independently recombining two separate
    pending lists against the untouched original on every Run click."""
    cob = cob_store or {}
    ...
    keys = [k for k in UNIQUE_KEY_COLS if k in d_orig.columns]
    if keys:
        if "COB_DATE" in d_orig.columns:
            d_orig = d_orig.sort_values("COB_DATE")
        d_orig = d_orig.drop_duplicates(subset=keys, keep="last")

    cob_date = resolve_cob_date(cob)
    current = replay_portfolio(d_orig, action_log or [], cob_date)
    return _df_to_records(current)
...
def _sim_result(run, reset, current_records, action_log,
    ...
    d_current = _records_to_df(current_records)
    if d_current.empty and not action_log:
        d_current = d_orig  # current-portfolio-store not populated yet — same as the original

    if d_orig.empty and d_current.empty:
        return html.P("No data.", style={"color": "#999", "padding": "20px", "textAlign": "center"}), no_update

    piv_orig  = build_pivot(d_orig, y_col, x_col, metric)
    piv_sim   = build_pivot(d_current, y_col, x_col, metric)
    ...
    fig_sim   = make_heatmap_fig(piv_sim,   f"Current Portfolio — {m_lbl}", "BuGn",  page, show_totals=True)
    ...
    if not paged_d.empty:
        ...
        fig_delta.update_layout(
            title={"text": f"Delta (Current Portfolio − Original) — {m_lbl}",
    ...
    caption_bits = [e.get("summary", "") for e in action_log]
    caption = " | ".join(caption_bits) if caption_bits else "No changes yet — Original and Current Portfolio are identical."
