...
def generate_view4_for_country(df: pd.DataFrame) -> pd.DataFrame:
    ...
    eoc_years = [y for y in eoc_years_raw if (_now_year - 15) <= y <= (_now_year + 20)]
    ...
    n_pairs = sum(1 for s in eoc_years for e in eoc_years if e >= s)
    print(f"    V4 {country_code}: {len(eoc_years)} EOC years [{eoc_years[0]}–{eoc_years[-1]}] "
          f"→ {n_pairs} pairs × {len(statuses)} statuses × {len(bike_or_cars)} bocs"
    ...
    for start_year in eoc_years:
        ...
        for end_year in eoc_years:
            ...
            # ── Hoist expensive ops ONCE per year pair (not per status/boc) ──────
            work = out_range.dropna(subset=["CONTRACT_FINAL_END", "MARKET_MODEL"])
            if work.empty:
                continue
            work, _, _ = fmd.build_period_column(work, "CONTRACT_FINAL_END", period_mode)
            work = work.copy()
            ...
            for status in statuses:
                for bike_or_car in bike_or_cars:
                    ...
                    grouped = sub.groupby(["PERIOD", "MARKET_MODEL"])["_tmp_key"].nunique().reset_index(name="COUNT")
        ...
        print(f"    V4 {country_code} start={start_year}: {pairs_done} pairs in {time.time()-t_start:.1f}s")
