...
def kpi_lease_under_25(
    ...
    subset = filter_base(df, country, year, status=status, bike_or_car=bike_or_car)

    if month_value and month_value != "ALL":
        
        subset = subset[subset["MONTH"].astype(str).str.lstrip("0") == str(month_value).lstrip("0")]

    if date_mode in ("CONTRACT_START_DATE", "DELIVERY_DATE"):
        subset = subset[
            (subset[date_mode].dt.month == subset["MONTH"]) &
            (subset[date_mode].dt.year == subset["YEAR"])
        ]
