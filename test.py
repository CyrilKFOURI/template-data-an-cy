...
DEFAULT_MODELS_PATH = BASE_DIR / "models.parquet"
...
COLUMNS_TO_READ = ['EXTENSION_DATE','CLASS_CATALOG','CONTRACT_START_DATE','CONTRACT_END_DATE','COB_DATE',
    ...
    'NOVA_ASSET_STATUS','FUEL_TYPE2','FUEL_TYPE','VA_CO2_EMSS_REAL','DELIVERY_DATE','MARKET_MODEL']
...
def get_kpi_data(df: pd.DataFrame, col: str | None = None, extra_keys: list[str] | None = None) -> pd.Series | pd.DataFrame:
    ...
    if missing:
        # If key columns are missing, we can't apply the logic
        return df[col] if col else df
...
def get_unique_key_series(df: pd.DataFrame) -> pd.Series:
    """Combines UNIQUE_KEY_COLS into a single string series for nunique() operations."""
    if df.empty:
        return pd.Series(dtype=str)
    keys = [c for c in UNIQUE_KEY_COLS if c in df.columns]
    if not keys:
        return df.index.to_series().astype(str)
    res = df[keys[0]].astype(str)
    for k in keys[1:]:
        res += "|" + df[k].astype(str)
    return res


def aggregate_unique_keys(df: pd.DataFrame) -> int:
    ...
    return len(df.drop_duplicates(subset=UNIQUE_KEY_COLS))
...
_MODELS_ENRICH_COLS = ["BRAND_UPDATE", "MODEL", "MARKET_MODEL", "MARKET_BODY_GROUP", "CDN_CLF_SEGMENT", "CDN_CLF_BODY_TYPE"]
...
def _load_models_df() -> pd.DataFrame:
    ...
    if not os.path.exists(path):
        return pd.DataFrame()
    try:
        mdf = pd.read_parquet(path, columns=_MODELS_ENRICH_COLS)
...
def enrich_with_models(df: pd.DataFrame) -> pd.DataFrame:
    """
    Si MARKET_MODEL ou MARKET_BODY_GROUP sont absents/vides, enrichit df depuis models.parquet.
    MODEL est extrait de CLASS_CATALOG (premier segment avant '/').
    """
    if MODELS_DF.empty:
        return df
    enrich_cols = ["MARKET_MODEL", "MARKET_BODY_GROUP", "CDN_CLF_SEGMENT", "CDN_CLF_BODY_TYPE"]
    already_present = all(c in df.columns and df[c].notna().any() for c in ["MARKET_MODEL", "MARKET_BODY_GROUP"])
    if already_present:
        return df
...
def normalize_market_model_value(brand_value: object, model_candidates: list[object]) -> str | None:
    ...
    for candidate in model_candidates:
        ...
        if brand_upper:
            ...
            cleaned = model
            for token in brand_variants:
                if token and model_upper.startswith(token):
                    cleaned = model[len(token):].strip(" -_/")
                    break
            if cleaned:
                return cleaned
...
def prepare_data_set(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"CONTRACT_END_DATE_AMENDED", "EXTENSION_DATE", "CONTRACT_END_DATE"}.intersection(df.columns):
        final_end_candidates = df.reindex(columns=["CONTRACT_END_DATE_AMENDED", "EXTENSION_DATE", "CONTRACT_END_DATE"]).apply(pd.to_datetime, errors="coerce")
    ...
    if "BRAND_UPDATE" in df.columns:
        df["MARKET_BRAND"] = df["BRAND_UPDATE"].astype(str).str.strip()
    else:
        df["MARKET_BRAND"] = pd.NA

    def infer_market_body_group_from_texts(texts: list[object]) -> str | None:
        if not texts:
            return None
        keywords_map = {
            "SUV": ["SUV", "SPORTUTILITY", "SPORT-UTILITY", "CROSS", "CROSSOVER"],
            "CITADINE": ["CITADINE", "CITY", "HATCH", "HATCHBACK", "MINI"],
            "VAN": ["VAN", "MINIVAN", "MPV", "CARVAN", "COMMERCIAL"],
        ...
        for t in texts:
            if t is None or pd.isna(t):
                continue
            s = str(t).upper()
            if s.strip() in {"PV", "LCV"}:
                continue
            for target, keys in keywords_map.items():
                for k in keys:
                    if k in s:
                        return target
        return None
    ...
    model_candidates = []
    for col in ["VEHICLE_MODEL", "MODEL_CATALOG", "CLASS_CATALOG", "VEHICLE_CLASS"]:
        if col in df.columns:
            model_candidates.append(df[col])

    inferred = []
    if model_candidates:
        for row_vals in zip(*[s.astype(object).tolist() for s in model_candidates]):
            inferred.append(infer_market_body_group_from_texts(list(row_vals)))

    if inferred and any(x is not None for x in inferred):
        df["MARKET_BODY_GROUP"] = pd.Series(inferred, index=df.index)
    else:
        body_group_source = pick_first_existing_column(df, ["CLS_VEHICLE_TYPE", "VEHICLE_CLASS", "BIKE_OR_CAR"])
        if body_group_source:
            df["MARKET_BODY_GROUP"] = df[body_group_source].astype(str).str.strip()
        else:
            df["MARKET_BODY_GROUP"] = pd.NA

    model_sources = [
        df["VEHICLE_MODEL"] if "VEHICLE_MODEL" in df.columns else pd.Series(index=df.index, dtype="object"),
        df["MODEL_CATALOG"] if "MODEL_CATALOG" in df.columns else pd.Series(index=df.index, dtype="object"),
        df["CLASS_CATALOG"] if "CLASS_CATALOG" in df.columns else pd.Series(index=df.index, dtype="object"),
    ]
    brand_source = df["BRAND_UPDATE"] if "BRAND_UPDATE" in df.columns else pd.Series(index=df.index, dtype="object")
    df["MARKET_MODEL"] = [
        normalize_market_model_value(brand_value, [model_value, model_catalog_value, class_catalog_value])
        for brand_value, model_value, model_catalog_value, class_catalog_value in zip(
            brand_source,
            model_sources[0],
            model_sources[1],
            model_sources[2],
...
def kpi_selected_volume(
    ...
    return aggregate_unique_keys(subset)
...
def kpi7_fuel_by_period(
    ...
    subset["_tmp_key"] = get_unique_key_series(subset)
    grouped = subset.groupby([fuel_col, "PERIOD_SORT", "PERIOD_LABEL"])["_tmp_key"].nunique().reset_index(name="VOLUME")
    if grouped.empty:
        return pd.DataFrame(), "", "", ""
...
def resolve_month_value(country: str, year: int | str, month_value: int | str) -> int | str:
    if month_value == "ALL":
        return "ALL"
...
def get_view1_metrics_cached(
    ...
    return {
        ...
        "month1_orig": kpi1_month, "month2_orig": kpi2_month, "month3_orig": kpi3_month,
        "month4_orig": kpi4_month, "month5_orig": kpi5_month, "month6_orig": kpi6_month,
...
def build_table(df_table: pd.DataFrame, page_size: int = 15, heatmap: bool = False, table_id: str = None) -> dash_table.DataTable:
    if table_id is None:
        # Generate a semi-random ID if none provided to avoid TypeError in newer Dash versions
        import time
        table_id = f"gen-table-{int(time.time() * 1000) % 1000000}"
...
def kpi8_production_ytd(
    ...
    # subset = get_kpi_data(subset) # Removed global key application
    ...
    # Apply unique key logic AFTER groupby using nunique
    subset["_tmp_key"] = get_unique_key_series(subset)
    grouped = (
        subset.groupby(["PERIOD", "POWER_CATEGORY_3"])["_tmp_key"]
        .nunique()
...
def build_period_column(df: pd.DataFrame, date_column: str, period_mode: str) -> tuple[pd.DataFrame, str, str]:
    subset = df.copy()
    date_values = pd.to_datetime(subset[date_column], errors="coerce")

    if period_mode == "monthly":
        subset["PERIOD"] = date_values.dt.strftime("%Y-%m")
        x_title = "Month"
        period_label = "Monthly"
    elif period_mode == "yearly":
        subset["PERIOD"] = date_values.dt.year.astype("Int64").astype(str)
        x_title = "Year"
        period_label = "Yearly"
    else:
        subset["PERIOD"] = date_values.dt.to_period("Q").astype(str)
        x_title = "Quarter"
        period_label = "Quarterly"

    return subset, x_title, period_label


def kpi9_1_type_share_period(
    df: pd.DataFrame,
    year: int | str,
    vehicle_type: str,
    country: str,
    ...
    if "YEAR" not in subset.columns:
        subset["YEAR"] = pd.to_datetime(subset["COB_DATE"], errors="coerce").dt.year
    if "MONTH" not in subset.columns:
        subset["MONTH"] = pd.to_datetime(subset["COB_DATE"], errors="coerce").dt.month
    ...
    if bike_or_car != "ALL" and "BIKE_OR_CAR" in subset.columns:
        subset = subset[subset["BIKE_OR_CAR"].astype(str).str.upper() == bike_or_car.upper()]

    body_group_col = pick_first_existing_column(subset, ["MARKET_BODY_GROUP", "CLS_VEHICLE_TYPE", "VEHICLE_CLASS"])
    if body_group_col is None:
        return pd.DataFrame(columns=["PERIOD", "PCT"])

    subset = subset.dropna(subset=["COB_DATE", body_group_col, "VEHICLE_ID"])
    if subset.empty:
        return pd.DataFrame(columns=["PERIOD", "PCT"])

    key_cols = [column for column in ["ID_QUOTATION", "ID_CONTRACT", "VEHICLE_ID"] if column in subset.columns]
    if key_cols:
        subset = subset.drop_duplicates(subset=key_cols)

    subset, x_title, period_label = build_period_column(subset, "COB_DATE", period_mode)
    subset[body_group_col] = subset[body_group_col].astype(str).str.strip().str.upper()
    vehicle_type_value = str(vehicle_type).strip().upper()

    total = subset.groupby("PERIOD")["VEHICLE_ID"].count()
    if vehicle_type_value == "ALL":
        selected = total.copy()
    else:
        selected = subset[subset[body_group_col] == vehicle_type_value].groupby("PERIOD")["VEHICLE_ID"].count()
    pct = (selected / total * 100).fillna(0).round(2)

    result = pct.reset_index(name="PCT")
    result["PERIOD"] = result["PERIOD"].astype(str)
    result = result.sort_values("PERIOD")
    result.attrs["x_title"] = x_title
    result.attrs["period_label"] = period_label
    result.attrs["body_group_col"] = body_group_col
    return result


def kpi9_2_power_category_per_type_period(
    df: pd.DataFrame,
    year: int | str,
    vehicle_type: str,
    country: str,
    ...
    body_group_col = pick_first_existing_column(subset, ["MARKET_BODY_GROUP", "CLS_VEHICLE_TYPE", "VEHICLE_CLASS"])
    if body_group_col is None or "POWER_CATEGORY" not in subset.columns:
        return pd.DataFrame()

    subset = subset.dropna(subset=["COB_DATE", body_group_col, "POWER_CATEGORY", "VEHICLE_ID"])
    if subset.empty:
        return pd.DataFrame()
    ...
    subset, x_title, period_label = build_period_column(subset, "COB_DATE", period_mode)
    subset[body_group_col] = subset[body_group_col].astype(str).str.strip().str.upper()
    selected_body = str(vehicle_type).strip().upper()
    if selected_body != "ALL":
        subset = subset[subset[body_group_col] == selected_body]
    if subset.empty:
        return pd.DataFrame()

    subset["POWER_CATEGORY"] = subset["POWER_CATEGORY"].astype(str).str.strip().str.upper()
    grouped = subset.groupby(["PERIOD", "POWER_CATEGORY"])['VEHICLE_ID'].count().reset_index(name="COUNT")
    pivot = grouped.pivot(index="PERIOD", columns="POWER_CATEGORY", values="COUNT").fillna(0)
    pivot = pivot.sort_index()
    pivot.attrs["x_title"] = x_title
    pivot.attrs["period_label"] = period_label
    return pivot
...
def kpi_count_share_quarterly(df: pd.DataFrame, asset_status: str, var_col: str, bike_or_car: str = "CAR", top_n: str = "1", period_mode: str = "quarterly") -> pd.DataFrame:
    ...
    out["_tmp_key"] = get_unique_key_series(out)
    grouped = out.groupby(["COUNTRY", var_col, "YEAR", "PERIOD"])["_tmp_key"].nunique().reset_index(name="VOLUME")
...
def kpi_count_share_ytd_by_quarter(df: pd.DataFrame, asset_status: str, var_col: str, bike_or_car: str = "CAR", top_n: str = "1", period_mode: str = "quarterly") -> pd.DataFrame:
    ...
    out["_tmp_key"] = get_unique_key_series(out)
    grouped = (
        out.groupby(["COUNTRY", var_col, "YEAR", "PERIOD"])["_tmp_key"]
        .nunique()
        .reset_index(name="VOLUME_YTD")
    )
...
def kpi_top_brand_vs_market(df_portfolio: pd.DataFrame, df_market: pd.DataFrame, var_col_portfolio: str, var_col_market: str, asset_status: str, reg_type_portfolio: list | str = "ALL", reg_type_market: list | str = "ALL", bev_only: bool = False, top_n: str = "ALL", period_mode: str = "quarterly") -> pd.DataFrame:
    ...
    # df_port = get_kpi_data(df_port, extra_keys=["PERIOD"]) # Removed
    ...
    df_port["_tmp_key"] = get_unique_key_series(df_port)
    port = (
        df_port.groupby(["COUNTRY", "PERIOD", var_col_portfolio])["_tmp_key"]
        .nunique()
...
def kpi_top_per_quarter_with_share(
    ...
    for country in sorted(out["COUNTRY"].dropna().unique().tolist()):
        ...
        for period in sorted(country_df["PERIOD"].dropna().unique().tolist()):
            ...
            counts = period_df.groupby(var_col).apply(aggregate_unique_keys).sort_values(ascending=False)
...
if os.environ.get("FAST_READER_MODE") == "1":
    df = pd.DataFrame()
...
if not df.empty and "COUNTRY" in df.columns:
    ...
    market_body_groups = sorted(df["MARKET_BODY_GROUP"].dropna().astype(str).str.upper().unique().tolist()) if "MARKET_BODY_GROUP" in df.columns else []
    ...
    has_vehicle_type_temp = not df.empty and ("MARKET_BODY_GROUP" in df.columns or "VEHICLE_TYPE_TEMP" in df.columns)
else:
    countries = []
    years = []
    statuses = []
    vehicle_col_global = None
    vehicle_types = []
    market_body_groups = []
    min_cob_date = pd.NaT
    max_cob_date = pd.NaT
    cob_date_options = []
    has_vehicle_type_temp = False
            ...
            children=[
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div("Country", className="filter-label", style={"marginTop": "10px", "fontWeight": "bold"}),
                        dcc.Dropdown(
                            id="report-modal-country",
                            options=[],
                            value=None,
                            clearable=False,
                            style={"width": "100%"}
                        ),
                        html.Div("Years to include", className="filter-label", style={"marginTop": "16px", "fontWeight": "bold"}),
                        dcc.Checklist(
                        ...
                        dcc.Dropdown(id="report-modal-date-mode", style={"display": "none"}, value="NONE"),
                        html.Div(
                        ...
                        [
                            ...
                            html.Div(
                                [
                                    ...
                                    # Hidden dummies kept so download callbacks referencing these IDs still work
                                    dcc.Dropdown(id="kpi7-start-date-filter", options=[], value=None, style={"display": "none"}),
                                    dcc.Dropdown(id="kpi7-end-date-filter",   options=[], value=None, style={"display": "none"}),
                                ],
                                className="kpi7-filter-box",
                            ),
                            html.Div(
        ...
        [
            html.Div(
                [
                    ...
                    html.Div(
                        [
                            html.H1("Vue 3 - Type mix"),
                            html.P("KPI 9_1 type share and KPI 9_2 power mix by period."),
                        ]
                    ),
                ],
                className="hero",
            ...
            html.Div(
                [
                    ...
                    html.Div(
                        [
                            html.Div("Country", className="filter-label"),
                            dcc.Dropdown(id="v3-country-filter-91", options=country_options, value=None, placeholder="Select country", clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Year", className="filter-label"),
                            dcc.Dropdown(id="v3-year-filter-91", options=year_options, value=None, placeholder="Select year", clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Asset Status", className="filter-label"),
                            dcc.Dropdown(id="v3-status-filter-91", options=asset_status_options, value=["IN FLEET"], multi=True, clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                    ...
                    html.Div(
                        [
                            html.Div("Vehicle class", className="filter-label"),
                            dcc.Dropdown(id="v3-vehicle-class-filter-91", options=["ALL"] + vehicle_types, value="ALL", placeholder="Select vehicle class", clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Vehicle body", className="filter-label"),
                            dcc.Dropdown(id="v3-vehicle-body-filter-91", options=["ALL"] + market_body_groups, value="ALL", placeholder="Select vehicle body", clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Bike or car", className="filter-label"),
                            dcc.Dropdown(id="v3-bike-or-car-filter-91", options=bike_or_car_options, value="CAR", clearable=False),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Action", className="filter-label"),
                            html.Button("Reload KPI 9_1", id="view3-refresh-button-91", n_clicks=0, className="primary-button"),
                        ],
                        className="filter-box",
                    ),
                ],
            ...
            html.Div(
                [
                    html.Div("KPI 9_1 - Type share", className="panel-title"),
            ...
            html.Div(
                [
                    ...
                    html.Div(
                        [
                            html.Div("Country", className="filter-label"),
                            dcc.Dropdown(id="v3-country-filter-92", options=country_options, value=None, placeholder="Select country", clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Year", className="filter-label"),
                            dcc.Dropdown(id="v3-year-filter-92", options=year_options, value=None, placeholder="Select year", clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Asset Status", className="filter-label"),
                            dcc.Dropdown(id="v3-status-filter-92", options=asset_status_options, value="IN FLEET", clearable=False),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                    ...
                    html.Div(
                        [
                            html.Div("Vehicle class", className="filter-label"),
                            dcc.Dropdown(id="v3-vehicle-class-filter-92", options=["ALL"] + vehicle_types, value="ALL", placeholder="Select vehicle class", clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Vehicle body", className="filter-label"),
                            dcc.Dropdown(id="v3-vehicle-body-filter-92", options=["ALL"] + market_body_groups, value="ALL", placeholder="Select vehicle body", clearable=True),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Bike or car", className="filter-label"),
                            dcc.Dropdown(id="v3-bike-or-car-filter-92", options=bike_or_car_options, value="CAR", clearable=False),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                        [
                            html.Div("Action", className="filter-label"),
                            html.Button("Reload KPI 9_2", id="view3-refresh-button-92", n_clicks=0, className="primary-button"),
                        ],
                        className="filter-box",
                    ),
                ],
            ...
            html.Div(
                [
                    html.Div("KPI 9_2 - Power mix by type", className="panel-title"),
                ...
                [
                    ...
                    html.Div(
                        [
                            ...
                            html.P("KPI 10 - Top models at End of Contract by period."),
                        ]
                    ),
                ],
                className="hero",
            ...
            html.Div(
                [
                    ...
                    html.Div(
                        [
                            html.Div("Bike or car", className="filter-label"),
                            dcc.Dropdown(id="v4-bike-or-car-filter", options=bike_or_car_options, value="CAR", clearable=False),
                        ],
                        className="filter-box",
                    ),
                    html.Div(
                ...
                [
                    ...
                    html.Div(
                        [
                            ...
                            html.Div(
                                [
                                    html.Div("Portfolio Status", className="filter-label", id="v5-status-filter-label"),
                                ...
                                className="filter-box",
                                id="v5-status-container"
                            ),
                            html.Div(
                                [
                                    html.Div("Market Registration", className="filter-label"),
                                    dcc.Dropdown(
                                        id="v5-market-reg-filter",
                                        options=[
...
def unified_time_and_card_sync(gran, country, year, month, date_mode, status, vehicle_type):
    ...
    return [opts, month, label, gran] + [opts]*6 + [month]*6 + [country]*6 + [year]*6 + [date_mode]*6 + [label]*6 + [status]*6 + [vehicle_type]*6
...
def update_kpi7(_refresh_clicks: int, _kpi7_refresh_clicks: int, country: str, year, status_group: str, metric_mode: str, period_mode: str, bike_or_car: str):
    start_date = f"{year}-01-01" if year and year != "ALL" else None
    end_date   = f"{year}-12-31" if year and year != "ALL" else None
...
def download_view1_html(
    ...
    try:
        html_content = build_view1_download_report(
            ...
            kpi_common_bike_or_car,
            kpi7_country, kpi7_year, kpi7_year,
            kpi7_status_group, kpi7_metric_mode, kpi7_period_mode,
            kpi7_bike_or_car,
            summary_data or [],
            kpi7_fig or {},
...
def update_view3_kpi91(
    _refresh_clicks: int,
    country: str,
    year: int | str,
    status_group: str | list[str] | None,
    period_mode: str,
    bike_or_car: str,
    vehicle_class: str,
    vehicle_body: str,
):
    if has_missing_filters(country, year, status_group, period_mode, bike_or_car):
        empty_fig = go.Figure()
    ...
    if vehicle_class and vehicle_class != "ALL":
        vcol = vehicle_col_global
        if vcol and vcol in local_df.columns:
            local_df = local_df[local_df[vcol].astype(str).str.upper() == str(vehicle_class).strip().upper()]

    selected_body = vehicle_body if vehicle_body is not None else "ALL"

    if isinstance(status_group, list):
        selected_statuses = [str(value).strip().upper() for value in status_group if value is not None and str(value).strip()]
    elif status_group in (None, ""):
        selected_statuses = []
    else:
        selected_statuses = [str(status_group).strip().upper()]

    if not selected_statuses:
        empty_fig = go.Figure()
        empty_fig.update_layout(title="KPI 9_1 - Select at least one Asset Status")
        return empty_fig, html.Div("Select at least one Asset Status then click Reload.", className="small-note")
    ...
    for idx, status_value in enumerate(selected_statuses):
        kpi91 = kpi9_1_type_share_period(local_df, year, selected_body, country, status_value, bike_or_car, period_mode)
        if kpi91.empty:
            continue

        x_axis_title = str(kpi91.attrs.get("x_title", "Period"))
        color = status_colors[idx % len(status_colors)]
        fig91.add_trace(
            go.Bar(
                x=kpi91["PERIOD"].tolist(),
                y=kpi91["PCT"].tolist(),
                name=f"{status_value} (bar)",
                marker_color=color,
                opacity=0.6,
                legendgroup=status_value,
            )
        )
        fig91.add_trace(
            go.Scatter(
    ...
    fig91.update_layout(
        title=f"KPI 9_1 - {vehicle_class} share ({year}, {', '.join(selected_statuses)}, {bike_or_car})",
    ...
    if not table_frames:
        ...
        table91 = pd.concat(table_frames, ignore_index=True)
        table91 = table91.rename(columns={"PERIOD": "Period", "PCT": "Share (%)", "ASSET_STATUS": "Asset Status"})
        pivot91 = table91.pivot_table(index="Asset Status", columns="Period", values="Share (%)", aggfunc="first")
        pivot91 = pivot91.reindex(index=selected_statuses)
        pivot91 = pivot91.reset_index()
        pivot91.columns.name = None
        table91_wrap = html.Div([html.H4("KPI 9_1 table", className="panel-title"), build_table(pivot91, page_size=15)])

    return fig91, table91_wrap
...
def update_view3_kpi92(_refresh_clicks: int, country: str, year: int | str, status_group: str, period_mode: str, bike_or_car: str, vehicle_class: str, vehicle_body: str):
    if has_missing_filters(country, year, status_group, period_mode, bike_or_car):
        empty_fig = go.Figure()
    ...
    selected_body = vehicle_body if vehicle_body is not None else "ALL"
    pivot92 = kpi9_2_power_category_per_type_period(local_df, year, selected_body, country, status_group, bike_or_car, period_mode)
    fig92 = go.Figure()
    if not pivot92.empty:
        x_title92 = str(pivot92.attrs.get("x_title", "Period"))
        period_values = pivot92.index.tolist()
        for c in pivot92.columns:
            fig92.add_trace(go.Scatter(x=period_values, y=pivot92[c].tolist(), mode="lines+markers", name=str(c)))
    fig92.update_layout(
        title=f"KPI 9_2 - {vehicle_class} power mix ({year}, {status_group}, {bike_or_car})",
        xaxis_title=x_title92 if not pivot92.empty else "Period",
        yaxis_title="Volume",
        template="plotly_white",
        height=420,
    )
...
def update_view4_kpi10(_refresh_clicks: int, country: str, year: int | str, status: str, period_mode: str, bike_or_car: str):
    ...
    model_col = "MARKET_MODEL"
    if model_col not in out.columns:
        empty_fig = go.Figure()
        empty_fig.update_layout(title="KPI 10 - MARKET_MODEL column is missing")
    ...
    period_subset, x_title, period_label = build_period_column(out, "CONTRACT_FINAL_END", period_mode)
    period_subset[model_col] = period_subset[model_col].astype(str).str.strip()
    ...
    period_subset["_tmp_key"] = get_unique_key_series(period_subset)
    grouped = period_subset.groupby(["PERIOD", model_col])["_tmp_key"].nunique().reset_index(name="COUNT")
    pivot = grouped.pivot(index="PERIOD", columns=model_col, values="COUNT").fillna(0)
    ...
    top_models_order = col_order[:10]
    pivot = pivot[top_models_order]
    kpi10_table = pivot.reset_index().rename(columns={"PERIOD": "Period"})
    ...
    if not quarters:
        ...
        top_models = top_models_order.tolist()
...
def update_view5_kpi11(_refresh_clicks: int, status_value: str, market_reg_value: list, source_value: str, variable: str, top_n_val: str, period_mode: str, market_owner: str):
    # Determine the relevant filter based on source
    active_status = market_reg_value if source_value == "market" else status_value
    
    if has_missing_filters(active_status, source_value, variable):
        empty_fig = go.Figure()
    ...
    if source_value == "market":
        # Filter market data by Registration Type (selected via the checklist)
        if not market_reg_value:
             m_filtered = market_df.iloc[0:0].copy()
        else:
             m_filtered = market_df[market_df["Registration Type"].isin(market_reg_value)].copy()
    ...
    table = build_table(kpi11_table, page_size=15, heatmap=True, table_id="v5-kpi11-table")
...
def update_view6_kpi13(_refresh_clicks: int, status_value: str, port_reg: list, mkt_reg: list, variable: str, period_mode: str, top_n: str, owner_type: str):
    ...
    # Define reg_type for the title (summary of portfolio and market registration types)
    reg_p = port_reg if isinstance(port_reg, str) else ", ".join(map(str, port_reg))
    reg_m = mkt_reg if isinstance(mkt_reg, str) else ", ".join(map(str, mkt_reg))
    reg_type = f"P:{reg_p} / M:{reg_m}"
