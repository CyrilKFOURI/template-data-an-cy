...
# Y axis options for simulation only (= notebook s_y — without SHARED_CLIENT_FLAG)
SIM_Y_OPTIONS = [
    {"label": "Brand",                      "value": "BRAND_UPDATE"},
    {"label": "Power Category",             "value": "POWER_CATEGORY"},
    {"label": "CO2 Bucket",                 "value": "CO2_BUCKET"},
...
def build_exposure_pivot(d: pd.DataFrame, y_col: str, x_col: str, aggregation: str = "sum") -> pd.DataFrame:
    ...
    d = d[cols].dropna(subset=[y_col, x_col])
...
GROUP_RATING_OPTIONS = (
    _sort_by_known_order(UC1_DF["GROUP_RATING"].dropna().unique().tolist(), RATING_GRADE_ORDER)
...
def apply_removal(d: pd.DataFrame, y_col: str, rem_dict: dict) -> pd.DataFrame:
    """Remove N existing rows per Y-axis category (arbitrary pick among rows already
    sharing that category — nothing about the category/metrics themselves is randomised)."""
    ...
    for y_val, n in rem_dict.items():
        ...
        if n <= 0:
            continue
        idx = out.index[out[y_col].astype(str) == str(y_val)].tolist()
        if idx:
            ...
            [
                html.Div(
                    [
                        ...
                        html.P("CAR portfolio · enriched with market model & body type · unique contract key dedup",
                            ...
                            children=[
                                html.Div(
                                    [
                                        ...
                                        html.Div(
                                            [
                                                _panel_title("Remove Existing Vehicles"),
                                                html.Div(
                                                    [
                                                        html.Label("Group By", style={
                                                            "fontSize": "11px", "fontWeight": "700",
                                                            "color": "#4a5568", "marginBottom": "4px", "display": "block"}),
                                                        dcc.Dropdown(
                                                            id="sim-y", options=SIM_Y_OPTIONS,
                                                            value=SIM_Y_OPTIONS[0]["value"],
                                                            clearable=False, style={"fontSize": "12px"}),
                                                    ],
                                                    style={"marginBottom": "12px"},
                                                ),
                                                html.Div(id="sim-controls", style={"maxHeight": "220px", "overflowY": "auto"}),
                                            ],
                                            style={"background": "#ffffff", "borderRadius": "8px",
                                                   "border": "1px solid #e2e8f0", "padding": "20px", "flex": "1"},
                                        ),
...
# =============================================================================
# Callbacks — Remove Existing Vehicles (per Y-category quantity)
# =============================================================================
...
def _sim_controls(_, page, y_col, country, asset_status, brands, models, bodytypes, cob_store):
        ...
        rows.append(html.Div(
            [
                ...
                dcc.Input(
                    ...
                    type="number", min=0, max=cnt, step=10, value=0,
...
def _sim_result(run, reset, rem_vals, rem_ids, batch_data,
    ...
    d_sim = apply_removal(d_orig, y_col, rem_dict)
    ...
    removed_parts = [f"−{int(n)} × {y_val}" for y_val, n in rem_dict.items() if int(n or 0) > 0]
