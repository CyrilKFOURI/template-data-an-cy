                ...
                [
                    ...
                    html.Div(
                        [
                            ...
                            html.Button("Complete Report", id="v1-open-complete-report", n_clicks=0, className="secondary-button"),
                        ],
                        className="filter-box",
                    ),
                ],
                ...
                [
                    ...
                    html.Div(
                        [
                            ...
                            html.Button("Complete Report", id="v2-open-complete-report", n_clicks=0, className="secondary-button"),
                        ],
                        className="filter-box",
                    ),
                ],
...
@app.callback(
    ...
    [
        Input('v1-open-complete-report', 'n_clicks'),
        Input('v2-open-complete-report', 'n_clicks'),
        Input('close-complete-report-modal', 'n_clicks')
    ],
    State('complete-report-modal', 'style'),
    prevent_initial_call=True
)
def toggle_complete_report_modal(n_v1, n_v2, n_close, current_style):
    import dash
    ctx = dash.callback_context
    if not ctx.triggered:
        return current_style
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]
    if button_id in ['v1-open-complete-report', 'v2-open-complete-report']:
        ...
        if (gran === 'monthly') {
            if (subSelect.options.length !== 12) {
                ...
                for(var i=1; i<=12; i++) {
                    ...
                    opt.value = i; opt.innerHTML = 'Month ' + i;
            ...
            if (subSelect.options.length !== 4) {
                ...
                for(var i=1; i<=4; i++) {
                    var opt = document.createElement('option');
                    opt.value = 'Q' + i; opt.innerHTML = 'Quarter ' + i;
                    subSelect.appendChild(opt);
                }
            }
        } else {
    ...
    for year in selected_years:
        for gran in selected_periods:
            ...
            for sub in subs:
                ...
                # --- CALCULATE DATA ---
                kpi1_val = kpi_lease_under_25(df, country, year, month_val, bike_or_car)
                kpi2_val = kpi_lease_25_30(df, country, year, month_val, bike_or_car)
                diesel_non = kpi_diesel_non_diesel(df, country, year, month_val, bike_or_car)
                hybrid_val = kpi_hybrid_share(df, country, year, month_val, bike_or_car)
                ev_val = kpi_ev_share(df, country, year, month_val, bike_or_car)
                pv_lcv_val = kpi_pv_lcv(df, country, year, month_val, bike_or_car)
                
                kpi7_pivot, y_title7, x_title7, period_label7 = kpi7_fuel_by_period(
                    df, country, status_group, "share", gran, bike_or_car, "COB_DATE", None, None
                )
                kpi7_fig = figure_from_pivot(
                ...
                table_kpi8, y_title8, x_title8, period_label8 = kpi8_production_ytd(
                    df, country, year, status_group, "volume", bike_or_car, "CONTRACT_START_DATE"
                )
                kpi8_fig = go.Figure()
                if not table_kpi8.empty:
                    x_values = table_kpi8["MONTH"].tolist()
                    ...
                    for col in ordered_metric_cols:
                        color = color_by_metric[col]
                        kpi8_fig.add_trace(go.Scatter(x=x_values, y=table_kpi8[col], mode="lines+markers", name=str(col), line={"color": color, "width": 2.2}, marker={"color": color, "size": 7}))
                kpi8_fig.update_layout(title=f"Production by fuel type (volume) by month<br><sup>{period_label8} | {country} | {bike_or_car}</sup>", xaxis_title=x_title8, yaxis_title=y_title8, template="plotly_white", height=520)

                kpi1_volume = kpi_selected_volume(df, country, year, month_val, status="IN FLEET", bike_or_car=bike_or_car)
                
                # --- BUILD HTML BLOCK ---
                block = f'<div class="report-block" data-year="{year}" data-gran="{gran}" data-sub="{sub}" style="display: none;">'
                block += f"<h2>Report for {country} - {year} ({gran} - {sub})</h2>"
                
                # KPI Cards (1-6)
                cards_html = build_kpi_cards_section(
                    kpi1_val, kpi2_val, diesel_non, hybrid_val, ev_val, pv_lcv_val,
                    desc, kpi1_limit, kpi2_limit, kpi1_volume, kpi1_volume, kpi1_volume, kpi1_volume, kpi1_volume, kpi1_volume, "volume"
                )
                block += f"<div class='section'>{cards_html}</div>"
                
                # Plotly Charts (7 & 8)
                import plotly.io as pio
                kpi7_html = pio.to_html(kpi7_fig, full_html=False, include_plotlyjs='cdn')
                kpi8_html = pio.to_html(kpi8_fig, full_html=False, include_plotlyjs='cdn')
                
                block += f"<div class='section'><h3>KPI 7 - Fuel by Period</h3>{kpi7_html}</div>"
                block += f"<div class='section'><h3>KPI 8 - Production YTD</h3>{kpi8_html}</div>"
