...
def _load_models() -> pd.DataFrame:
    ...
    if not os.path.exists(path):
        ...
        return pd.DataFrame()
    try:
        df = pd.read_parquet(path, columns=_MODELS_COLS)
...
def enrich_with_models(df: pd.DataFrame) -> pd.DataFrame:
    """
    Merge NOVA data with models.parquet to obtain MARKET_MODEL, MARKET_BODY_GROUP,
    CDN_CLF_SEGMENT, CDN_CLF_BODY_TYPE.
    MODEL is extracted from CLASS_CATALOG as the first segment before '/'.
    """
    if MODELS_DF.empty or "CLASS_CATALOG" not in df.columns or "BRAND_UPDATE" not in df.columns:
        return df
    out = df.copy()
    out["MODEL"] = out["CLASS_CATALOG"].astype(str).str.split("/").str[0].str.strip()
    # Drop columns that will come from the merge to avoid _x/_y suffixes
...
def generate_view4_for_country(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Vue 4 rule: CONTRACT_FINAL_END = first non-null among (AMENDED → EXTENSION → END)
