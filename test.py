...
df['POWER_CATEGORY_2'] = np.where(
    df['POWER_CATEGORY'].astype(str).str.strip().str.upper().eq('NHEV'),
...
df['POWER_CATEGORY_3'] = np.select(
    [
        df['POWER_CATEGORY'].astype(str).str.strip().str.upper().eq('MHEV'),
        df['POWER_CATEGORY'].astype(str).str.strip().str.upper().eq('PLUG-IN HYBRID')
    ],
    [
        'MHEV-' + fuel_clean,
        'PHEV-' + fuel_clean
...
# fallback sécurité
df['POWER_CATEGORY_2'] = df['POWER_CATEGORY_2'].fillna(df['POWER_CATEGORY'])
