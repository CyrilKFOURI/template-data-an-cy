...
def resolve_logo_path() -> Path | None:
    ...
    try:
        if LOGO_PATH.is_file() and LOGO_PATH.suffix.lower() in exts:
            return LOGO_PATH
    except OSError:
        pass
...
@lru_cache(maxsize=1)
def prepare_dash_logo_asset() -> str:
    logo_path = resolve_logo_path()
    if logo_path is None:
        return ""

    assets_dir = BASE_DIR / "fleet_assets"
    assets_dir.mkdir(parents=True, exist_ok=True)
    target_name = f"runtime_logo{logo_path.suffix.lower()}"
    target_path = assets_dir / target_name

    try:
        shutil.copyfile(logo_path, target_path)
    except OSError:
        return ""
...
def load_logo_src() -> str:
    runtime_logo = prepare_dash_logo_asset()
    if not runtime_logo:
        return ""
    return f"/assets/{runtime_logo}"
...
def html_logo_block(class_name: str = "report-logo") -> str:
    ...
    if logo_path is None:
        return ""
    return f'<img class="{class_name}" src="{html_escape(logo_path.resolve().as_uri())}" alt="Fleet Monitoring logo" />'
