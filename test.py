        ...
        for period_mode in period_modes:
            for bike_or_car in bike_or_cars:
                for vehicle_class in vehicle_types:
                    ...
                    # ── Build PERIOD column ONCE for this (year, mode, bike, class) ──
                    work = local_df.dropna(subset=["COB_DATE", body_group_col, "VEHICLE_ID"]).copy()
                    if work.empty:
                        continue
                    work, _, _ = fmd.build_period_column(work, "COB_DATE", period_mode)
                    ...
                    # ── Precompute kpi9_1 base aggregation (replaces ~50k function calls) ──
                    agg91 = (
                        work.groupby(["PERIOD", body_group_col, "NOVA_ASSET_STATUS"], observed=True)["VEHICLE_ID"]
                        .nunique()
                    ...
                    if has_power:
                        ...
                        agg92 = (
                            work92.groupby(
                                ["PERIOD", body_group_col, "NOVA_ASSET_STATUS", "POWER_CATEGORY"], observed=True
                            )["VEHICLE_ID"]
                            .nunique()
                            .reset_index(name="COUNT")
                            .rename(columns={body_group_col: "BODY_GROUP"})
                    ...
                    # ── Derive kpi9_1 for all body × status combos via fast slicing ──
                    for vehicle_body in market_bodies:
                        ...
                        for selected_statuses in status_subsets:
                            ...
                            for status_value in status_list:
                                sv = str(status_value)
                                result = _kpi91_from_agg(agg91, body_val, sv)
                                if result.empty:
                                    continue
                        ...
                        if not agg92.empty:
                            for status_value in status_values:
                                result = _kpi92_from_agg(agg92, body_val, status_value)
                                if result.empty:
                                    continue
...
def generate_view4_for_country(df: pd.DataFrame) -> pd.DataFrame:
    ...
    eoc_years = sorted(eoc_year_series.unique().tolist())
    if not eoc_years:
        return pd.DataFrame()
    ...
    period_mode = "yearly"  # view 4 always shows volumes per year

    # ── Pre-filter country + parse datetime ONCE (not repeated 500+ times in loops) ──
    ...
    for start_year in eoc_years:
        for end_year in eoc_years:
            if end_year < start_year:
                continue
            out_range = out_base[out_base["_eoc_year"].between(start_year, end_year)]
            if out_range.empty or "MARKET_MODEL" not in out_range.columns:
                continue
            for status in statuses:
                for bike_or_car in bike_or_cars:
                    ...
                    table["START_YEAR_FILTER"]   = start_year
                    table["END_YEAR_FILTER"]     = end_year
