...
def _customer_exposure(customer_rows: pd.DataFrame) -> float:
    """Exposure is a customer-level attribute (repeated on every one of the
    customer's rows), so it must be taken once — not summed per vehicle."""
    if customer_rows.empty:
        return 0.0
...
VEHICLE_DETAIL_SECTIONS: list[tuple[str, list]] = [
    ("Identification", [
        ...
        ("MARKET_MODEL", "VEHICLE_MODEL", "MODEL_CATALOG"),
        "VEHICLE_CLASS",
...
def _vehicle_card(v: dict):
    ...
    model = v.get("MARKET_MODEL") or v.get("VEHICLE_MODEL") or v.get("MODEL_CATALOG") or "—"
    ...
    return html.Div(
        ...
        id={"type": "vehicle-card", "index": str(vid)}, n_clicks=0,
        style={**CARD_STYLE, "cursor": "pointer", "transition": "box-shadow .15s"},
    )
...
def _render_vehicle_detail(v: dict):
    ...
    brand = v.get("BRAND_UPDATE") or ""
    model = v.get("MARKET_MODEL") or v.get("VEHICLE_MODEL") or ""
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        dcc.Input(
                            id="customer-id-input", type="text", placeholder="Enter a Customer ID…",
                            debounce=False, n_submit=0,
                            style={"flex": "1", "maxWidth": "360px", "padding": "9px 12px",
...
def _search_customer(_n_clicks, _n_submit, cust_id):
    ...
    if customer_rows.empty:
        return (None, [], f'No customer found for ID "{cust_id}".',
                {**ERROR_STYLE, "display": "block"}, {"display": "none"}, {"display": "block"})
...
def _render_grid(vehicles, brands_f, bodytypes_f, statuses_f, sort_dir):
    ...
    cards = [_vehicle_card(row) for row in d.to_dict("records")]
...
@app.callback(
    ...
    Input({"type": "vehicle-card", "index": ALL}, "n_clicks"),
