...
def get_view1_metrics_cached(
    ...
    if granularity == "yearly":
        month1, month2, month3, month4, month5, month6 = "ALL", "ALL", "ALL", "ALL", "ALL", "ALL"
    else:
        month1 = resolve_month_value(kpi1_country, kpi1_year, kpi1_month)
        month2 = resolve_month_value(kpi2_country, kpi2_year, kpi2_month)
    ...
    return {
        ...
        "month1": month1,
        "month2": month2,
        "month1_orig": kpi1_month,
        "month2_orig": kpi2_month,
        "month3_orig": kpi3_month,
        "month4_orig": kpi4_month,
        "month5_orig": kpi5_month,
        "month6_orig": kpi6_month,
...
def build_table(df_table: pd.DataFrame, page_size: int = 15, heatmap: bool = False) -> dash_table.DataTable:
...
def build_view1_download_report(
    ...
    dm1 = kpi1_dm if kpi1_dm != "NONE" else date_mode
    dm2 = kpi2_dm if kpi2_dm != "NONE" else date_mode
    dm3 = kpi3_dm if kpi3_dm != "NONE" else date_mode
    dm4 = kpi4_dm if kpi4_dm != "NONE" else date_mode
    dm5 = kpi5_dm if kpi5_dm != "NONE" else date_mode
    dm6 = kpi6_dm if kpi6_dm != "NONE" else date_mode

    kpi1_volume = kpi_selected_volume(df, kpi1_country, kpi1_year, kpi1_month, status=kpi1_status or "IN FLEET", bike_or_car=vc1, date_mode=dm1)
    kpi2_volume = kpi_selected_volume(df, kpi2_country, kpi2_year, kpi2_month, status=kpi2_status or "IN FLEET", bike_or_car=vc2, date_mode=dm2)
    kpi3_volume = kpi_selected_volume(df, kpi3_country, kpi3_year, month3, status=kpi3_status or "IN FLEET", bike_or_car=vc3, date_mode=dm3)
    kpi4_volume = kpi_selected_volume(df, kpi4_country, kpi4_year, month4, status=kpi4_status or "IN FLEET", bike_or_car=vc4, date_mode=dm4)
    kpi5_volume = kpi_selected_volume(df, kpi5_country, kpi5_year, month5, status=kpi5_status or "IN FLEET", bike_or_car=vc5, date_mode=dm5)
    kpi6_volume = kpi_selected_volume(df, kpi6_country, kpi6_year, month6, status=kpi6_status or "IN FLEET", bike_or_car=vc6, date_mode=dm6)

    kpi1_val = kpi_lease_under_25(df, kpi1_country, kpi1_year, kpi1_month, vc1, dm1, kpi1_status or "IN FLEET")
    kpi2_val = kpi_lease_25_30(df, kpi2_country, kpi2_year, kpi2_month, vc2, dm2, kpi2_status or "IN FLEET")
    diesel_non = kpi_diesel_non_diesel(df, kpi3_country, kpi3_year, month3, vc3, dm3, kpi3_status or "IN FLEET")
    hybrid_val = kpi_hybrid_share(df, kpi4_country, kpi4_year, month4, vc4, dm4, kpi4_status or "IN FLEET")
    ev_val = kpi_ev_share(df, kpi5_country, kpi5_year, month5, vc5, dm5, kpi5_status or "IN FLEET")
    pv_lcv_val = kpi_pv_lcv(df, kpi6_country, kpi6_year, month6, vc6, dm6, kpi6_status or "IN FLEET")
    ...
    for kid, config in KPI_REGISTRY.items():
        ...
        period_lbl = summary_month_label(year_p, None if month_p in (None, 'ALL') else month_p)
...
def build_view2_download_report(
    ...
    table_kpi8, y_title, x_title, period_label = kpi8_production_ytd(df, country, year, asset_status, metric_mode, bike_or_car, v2_date_mode_filter, period_mode)
...
def update_kpi_cards(
    ...
    kpi1_val = kpi_lease_under_25(df, kpi1_country, kpi1_year, m1, vc1, dm1, kpi1_status or "IN FLEET")
    kpi2_val = kpi_lease_25_30(df, kpi2_country, kpi2_year, m2, vc2, dm2, kpi2_status or "IN FLEET")
    diesel_val = kpi_diesel_non_diesel(df, kpi3_country, kpi3_year, m3, vc3, dm3, kpi3_status or "IN FLEET")
    hybrid_val = kpi_hybrid_share(df, kpi4_country, kpi4_year, m4, vc4, dm4, kpi4_status or "IN FLEET")
    ev_val = kpi_ev_share(df, kpi5_country, kpi5_year, m5, vc5, dm5, kpi5_status or "IN FLEET")
    pv_lcv_val = kpi_pv_lcv(df, kpi6_country, kpi6_year, m6, vc6, dm6, kpi6_status or "IN FLEET")
...
def download_view2_html(_n_clicks: int, country: str, year: int | str, metric_mode: str, bike_or_car: str, v2_date_mode_filter: str, period_mode: str):
    ...
    html_content = build_view2_download_report(country, year, asset_status, metric_mode, bike_or_car, v2_date_mode_filter, period_mode)
    safe_country = str(country).replace(" ", "_")
    safe_year = str(year)
    safe_metric = str(metric_mode)
    safe_period = str(period_mode)
    filename = f"fleet_monitoring_vue2_{safe_country}_{safe_year}_{safe_metric}_{safe_period}.html"
    return dcc.send_string(html_content, filename=filename)
