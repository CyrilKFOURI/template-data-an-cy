...
COUNTRIES_TO_RUN = ALL_COUNTRIES
...
def save_parquet_merge(df: pd.DataFrame, path: str) -> None:
    ...
    if os.path.exists(path):
        try:
            ...
            # Detect country column — views 3/4/6 use COUNTRY_FILTER, views 1/5 use COUNTRY
            country_col = next(
                (col for col in ("COUNTRY_FILTER", "COUNTRY")
                 if col in existing.columns and col in df.columns),
                None,
            )
            if country_col is not None:
                new_countries = set(df[country_col].dropna().unique())
                n_before = len(existing)
                existing = existing[~existing[country_col].isin(new_countries)]
                print(f"    merge {os.path.basename(path)}: removed {n_before - len(existing):,} rows "
                      f"for {new_countries}, appending {len(df):,} new rows")
...
def generate_view6_for_country(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Maps every possible NOVA COUNTRY code/name to the exact Country/Territory-Name
    # used in the market dataset.  Countries absent from this map (or whose mapped
    # name is not in the market) are skipped with a printed message below.
    country_map = {
        # Short ISO codes
        "ES": "Spain",   "IT": "Italy",   "UK": "United Kingdom",
        "DE": "Germany", "FR": "France",  "BE": "Belgium",
        "NL": "Netherlands", "LU": "Luxembourg",
        # Full-name variants sometimes stored in NOVA
        ...
        # GB / Great Britain alias used in some market datasets
        "GB": "United Kingdom",    "GREAT BRITAIN": "United Kingdom",
    }

    # Early exit if this country has no corresponding market data
    mapped_country_name = country_map.get(str(country_code).strip().upper(), None)
    available_mkt_countries = set(MARKET_DF["Country/Territory-Name"].dropna().unique())
    if mapped_country_name is None or mapped_country_name not in available_mkt_countries:
        print(f"  View 6 | {country_code}: no market data available — skipping")
        return pd.DataFrame()
...
def run(countries: list[str]) -> None:
    ...
    # NOTE: view6 saves per-country immediately — no accumulation list needed
    ...
    for country in countries:
        ...
        if CONFIG["GENERATE_VIEW6"]:
            ...
            if not v6.empty:
                path_v6 = os.path.join(BASE_OUT, "view6", "kpi13.parquet")
                save_parquet_merge(v6, path_v6)
                print(f"  View 6 | {country}: {len(v6):,} rows merged into kpi13.parquet")
            else:
                print(f"  View 6 | {country}: generated 0 rows — parquet NOT updated")
    ...
    if view3_kpi91_frames:
        save_parquet_merge(pd.concat(view3_kpi91_frames, ignore_index=True), os.path.join(BASE_OUT, "view3", "kpi9_1.parquet"))
    if view3_kpi92_frames:
        save_parquet_merge(pd.concat(view3_kpi92_frames, ignore_index=True), os.path.join(BASE_OUT, "view3", "kpi9_2.parquet"))
    if view4_frames:
        save_parquet_merge(pd.concat(view4_frames, ignore_index=True), os.path.join(BASE_OUT, "view4", "kpi10.parquet"))
    ...
    # view6: already saved per-country inside the loop above
